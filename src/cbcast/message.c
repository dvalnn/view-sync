...
Result *cbc_msg_deserialize(const char *bytes) {
  if (!bytes) {
    return result_new_err("Null input bytes");
  }

  // Allocate memory for the message structure
  cbcast_msg_t *msg = calloc(1, sizeof(cbcast_msg_t));
  if (!msg) {
    return result_new_err("Memory allocation failed for message");
  }
  ...
  if (!msg->header) {
    free(msg);
    return result_new_err("Memory allocation failed for header");
  }
  ...
  if (msg->header->len == 0) {
    free(msg->header);
    free(msg);
    return result_new_err("Invalid header length");
  }
  ...
  if (!msg->payload) {
    free(msg->header);
    free(msg);
    return result_new_err("Memory allocation failed for payload");
  }
  memcpy(msg->payload, bytes + sizeof(cbcast_msg_hdr_t), msg->header->len);
  msg->payload[msg->header->len] = '\0'; // Ensure null termination

  // Debug output
  printf("[cbc_msg_deserialize] Deserialized message: kind=%d, pid=%d, "
         "clock=%d, len=%d, payload=%s\n",
         msg->header->kind, msg->header->pid, msg->header->clock,
